...
export class GameEngine {
	...
	private resetBall() {
		const angle = (Math.random() * Math.PI / 2) - (Math.PI / 4); // random angle between -45° and +45° (in radians)
		const direction = ((this.scoreP1 + this.scoreP2) % 2 === 0) ? 1 : -1; // send to the left or to the right
	...
	private moveBall() {
		...
		this.checkScore();
		if (this.ball.y - this.ball.radius <= 0 || this.ball.y + this.ball.radius >= this.field.height) // change trajectory after collision
			this.ball.speedY *= -1;
	}
	...
	private calculateBounce(player: Player, direction: 1 | -1) {
		const relativeY = (this.ball.y - (player.y + player.height / 2)); // where the ball hits the paddle
		const normalizedY = relativeY / (player.height / 2); // value between -1 and 1 depending on whether it hits the top, middle, or bottom of the paddle
		const maxAngle = Math.PI / 3; // max return angle
		const angle = normalizedY * maxAngle; // computed angle
		const maxSpeed = 14;
	...
	update(deltaTime: number = 16) {
		...
		// AI decision (max 1/sec) + movement every frame
		...
		// autosave is useless now -> we can remove this whole block
	}
}
