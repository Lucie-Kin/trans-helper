                            ...
                            <div className="round-matches">
                                {roundMatches.map((match) => (
                                    <div key={match.id} className="bracket-match">
                                        <div className={`bracket-player ${match.winner === match.playerA?.id ? "winner" : ""}`}>
                                            {match.playerA ? (
                                                <>
                                                    {match.playerA.isAI && <span className="ai-badge">🤖</span>}
                                                    <span className={match.playerA.confirmed ? "" : "uncomfirmed"}>
                                                        {match.playerA.name}
                                                    </span>
                                                </>
                                            ) : (
                                        ...
                                        <div className={`bracket-player ${match.winner === match.playerB?.id ? "winner": ""}`}>
                                            {match.playerB ? (
                                                <>
                                                    {match.playerB.isAI && <span className="ai-badge">🤖</span>}
                                                    <span className={match.playerB.confirmed ? "" : "uncomfirmed"}>
                                                        {match.playerB.name}
                                                    </span>
                                                </>
                                            ) : (
